...
- ☕ **حمایت مالی (Coffeete)**: نوار پیشرفت اهداف + لیست حامیان اخیر + مدیریت حامیان (ویرایش/مخفی‌سازی)
...
- ✏️ **ویرایشگر لایو**: ویرایش متن‌های ثابت سایت مستقیماً از صفحه عمومی (مثل المنتور)
- 📋 **سیستم لاگ‌گیری**: ثبت خودکار تمام عملیات ادمین + پاکسازی خودکار ۲۴ ساعته
...
3. محتوای `supabase/migrations/20260827000000_logs_and_overrides.sql` را کپی و **Run** کنید (جدول لاگ‌ها + ترجمه‌های سفارشی).

> ⚠️ این فایل شامل `DROP POLICY IF EXISTS` هست، پس اگر قبلاً ران شده باشد مشکلی پیش نمی‌آید.

4. در **Storage → New Bucket**: نام `covers`، تیک **Public bucket** روشن.
...
- **ویرایشگر لایو**: با زدن دکمه شناور «ویرایش درجا»، می‌توانید متن‌های ثابت سایت را مستقیماً از صفحه عمومی تغییر دهید
- **لاگ‌گیری**: تمام عملیات ادمین (ایجاد/ویرایش/حذف) ثبت می‌شود و در تب «لاگ‌های سیستم» قابل مشاهده است
